...
def _multiplex_fetch_as_dataframe(origin, fetches, sorter, cross_fetches=False, chain_fetches=False, flatten=True):
    ...
    index = _get_multiplex_dataframe_index(origin, fetches, sorter, cross_fetches=cross_fetches, chain_fetches=chain_fetches)
    assert False
    ...
    # Fetch origin data to be used in multiplex fetching data from other tables
    origin_data, data = _multiplex_fetch(origin['tables'],
                                         origin['attribute_names'],
                                         fetches['tables'][0],
                                         fetches['attribute_names'][0],
    ...
    # Store origin data as dataframe index
    data_dict['index'] = [(origin['table_names'], item) for item in origin_data]
    # Store first fetch data
    data_dict['data'][0] = data

    data_df_list = [None]*len(fetches['attribute_names'])
    # Fetch columns from tables based on previously fetched data from a different table
    for ndx in range(len(data_df_list)):
        if ndx == 0:
            origin_data = None
            previous_table = origin['tables']
            previous_attribute_name = origin['attribute_names']
            sort_table = sorter['tables']
            sort_attribute_names=sorter['attribute_names']
        else:
            sort_table = None
            sort_attribute_names = None
            if chain_fetches:
                previous_table = fetches['tables'][ndx-1]
                previous_attribute_name = fetches['attribute_names'][ndx-1]
            else:
                previous_table = origin['tables']
                previous_attribute_name = origin['attribute_names']
        # Fetch conformed data
        index, data = _multiplex_fetch(previous_table,
                                       previous_attribute_name,
                                       fetches['tables'][ndx],
                                       fetches['attribute_names'][ndx],
                                       sort_table=sort_table,
                                       sort_attribute_names=sort_attribute_names,
                                       origin_data=origin_data,
                                       cross_fetches=cross_fetches,
                                       chain_fetches=chain_fetches)
        # Store fetch and origin data in a dataframe
        df_dict = {'index' : [(origin['table_names'], item) for item in index],
                   'columns' : [([fetches['table_names'][ndx], fetches['attribute_names'][ndx]])],
                   'data' : data,
                   'index_names' : ['origin_table_name', origin['attribute_names']],
                   'column_names' : ['table_name', 'attribute_name']}
        data_df_list[ndx] = pd.DataFrame.from_dict(df_dict, orient='tight')
        #data_df_list[ndx] = pd.DataFrame(data=data, index=index, columns=[fetches['attribute_names'][ndx]])
    # Transpose data
    ...
    data_df = _merge_multiplex_fetch_dataframes(data_df_list, origin, fetches, cross_fetches=cross_fetches, chain_fetches=chain_fetches, flatten=flatten)
    return data_df

def _get_multiplex_dataframe_index(origin, fetches, sorter, cross_fetches=False, chain_fetches=False):

    # Get unique origin data
    origin_data_unique = unique_data(fetch(origin['tables'], origin['attribute_names']))
    origin_data_idx = 0
    origin_data_length = 0
    # Compare the length of each query on each fetch table and use the largest for creating the index
    for ndx in range(len(fetches['tables'])):
        if not cross_fetches and not chain_fetches:
            fetch_query = fetches['tables'][ndx] & sql_or_query(origin['attribute_names'], origin_data_unique)

        if len(fetch_query) > origin_data_length:
            origin_data_idx = ndx
            origin_data_length = len(fetch_query)
            origin_data = fetches
    # Fetch the attribute that will become the index
    origin_data = fetch(fetches['tables'][origin_data_idx], origin['attribute_names'])
        
    
    # Use the largest fetched dataset as the index 
    #idx = np.argmax([len(data) for data in origin_data_list])
    #origin_data = origin_data_list[idx]
    # Sort index according to sort table and sort attributes
    attribute_names = list(set( parse_iterable_inputs(sorter['attribute_names']) + parse_iterable_inputs(origin['attribute_names']) ))
    sort_df = fetch_as_dataframe(sorter['tables'], attribute_names=attribute_names)
    sort_df = sort_dataframe(sort_df, sort_columns=sorter['attribute_names'])
    sort_data = unique_data(sort_df[origin['attribute_names']].values.tolist())
    # Recreate index in the order specified by the sorted data
    origin_data = [[value]*origin_data.count(value) for value in sort_data]
    origin_data = list(itertools.chain(*origin_data))
    assert False
    return origin_data
    
    

def _merge_multiplex_fetch_dataframes(data_df_list, origin, fetches, cross_fetches=False, chain_fetches=False, flatten=True):

    data_df = data_df_list[0]
    if not cross_fetches and not chain_fetches:
        assert False
        data_df = data_df.join(data_df_list[1:], how='left')
    return data_df


def _multiplex_fetch_as_dataframe_OLD(origin, fetches, sorter, cross_fetches=True, chain_fetches=False, flatten=True):
    ...
    # Fetch columns from tables based on previously fetched data from a different table
    for ndx in range(len(data_dict['data'])):
        if ndx == 0:
            continue
        if ndx == 0 or not chain_fetches:
            ...
            previous_attribute_name = fetches['attribute_names'][ndx-1]
        # Fetch conformed data
        _, data = _multiplex_fetch(previous_table,
                                   previous_attribute_name,
                                   fetches['tables'][ndx],
                                   fetches['attribute_names'][ndx],
                                   origin_data=origin_data,
...
def _multiplex_fetch(origin_table, origin_attribute_name, fetch_table, fetch_attribute_names,
                     sort_table=None, sort_attribute_names=None, origin_data=None, cross_fetches=False, chain_fetches=False):
    ...
    if origin_data is None:
        ...
        if sort_table is not None and sort_attribute_names is not None:
            ...
            # Get sorted source dsort_attribute_names
