...
class NWBConverter(DataWriter):
    ...
    def _convert_rec_to_nwb(self, date, nwb_full_name):
        ...
        nwb_full_names_list = self._file_info.search_matching_file_names(".nwb").full_name.tolist()
        ...
        builder = self._get_raw_to_nwb_builder(date, metadata, trodes_rec_export_args=trodes_rec_export_args)
    ...
    def _get_raw_to_nwb_builder(self, date, metadata, trodes_rec_export_args=None):
        ...
        video_path = self._file_info._file_name_helper._get_paths_by_file_type("video")[0]
    ...
    def _nwb_converter_file_handler(self, date, nwb_full_name):
        ...
        rename_nwb_file(os.path.join(nwb_path, old_name), os.path.join(nwb_path, new_name))
    ...
    def _create_nwb_symlink(self, nwb_full_name):
        ...
        NWBConverter._create_symlink_target_directory(nwb_full_name, spyglass_nwb_path, overwrite=self._overwrite)
        print_verbose(self._verbose,
                      f"Symlink {nwb_full_name} to {spyglass_nwb_path}")
    ...
    def _create_video_symlink(self, video_full_name):
        ...
        NWBConverter._create_symlink_target_directory(video_full_name, spyglass_video_path, overwrite=self._overwrite)
        print_verbose(self._verbose,
                      f"Symlink {video_full_name} to {spyglass_video_path}")

    @staticmethod
    def _create_symlink_target_directory(source_full_name, destination_path, overwrite=False):
        ...
        if destination_file_name in [file.name for file in os.scandir(destination_path) if file.is_file()]:
            if not overwrite:
                no_overwrite_handler_file(f"File '{destination_file_name}' already exists in '{destination_path}'")
            else:
        ...
        assert destination_file_name in [file.name for file in os.scandir(destination_path) if file.is_file()], \
